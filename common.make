...
# It may override PREFIX, HTTP_PORT, ... and etc below here ?=
...
# PREFIX is a good variable to change.
PREFIX ?= $(HOME)/installed/mirrorworlds
...
# @HTTP_PORT@ with the value of $(HTTP_PORT) in "foo.in" to make
# file "foo".
seds :=\
 HTTP_PORT\
 HTTPS_PORT
...
installed := $(sort\
 $(downloaded)\
 $(INSTALLED)\
 $(wildcard *.js *.css *.html *.gif *.jpg *.png *.json *.csv *.md *.x3d)\
 $(filter-out %.jsp,$(filter-out %.cs,$(filter-out %.in,$(built))))\
)

# BUILD is not necessarily installed
built := $(sort $(built) $(BUILD))

cleanfiles := $(sort $(built) $(CLEANFILES))
cleanerfiles = $(sort $(CLEANERFILES))

ifndef ($(strip $(top_srcdir)),.)
    cleanerfiles := $(sort $(cleanerfiles) config.make)
endif
...
# running configure may have generated config.make
# and if configure was not run this will make a
# config.make file with the default values that
# are set near the top of this file.
