...
function ws_OnConnection(socket, ws) {
    ...
    socket.sendPayload = function(subscription) {
        if(subscription.payload && socket.readyState === webSocket.OPEN)
    ...
    on('get', function(clientkey, name,
        ...
        if(className) {
            ...
            // Send to all clients but socket.
    ...
    on('destroy', function(id) {
        ...
        subscription[id].destroy();
    });


    on('subscribe', function(id) {
    ...
    on('makeOwner', function(id, isOwner) {

        if(subscription[id] === undefined) return;
        subscription[id].makeOwner(isOwner);
    });
