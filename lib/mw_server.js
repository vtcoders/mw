    ...
    function send(clients) {
        clients.forEach(function(client) {
            if(client.readyState === webSocket.OPEN &&
                    // Exclude this webSocket client
                    // if this is set to a client
                    this !== client)
                client.send(data);
        });
    };
...
function ws_OnConnection(socket) {
    ...
    function newSubscription(name, className, shortName, description,
        ...
        var s = {
            ...
            makeOwner: function(sock=null, isOwner=true) {
                // add a webSocket reader to the list
                if(!sock) var sock = socket;
            ...
            destroy: function() {
                assert(this.id);
                if(this.name) {
                    assert(subscription[this.name] !== undefined,
                        'cannot delete subscription name=' + this.name);
                    delete subscription[this.name];
                }
        ...
        if(name) {
            ...
            // Note this is by reference and not an object copy.
    ...
    on('get', function(clientkey, name,
        ...
        socket.Emit('get', s.id, clientkey, s.name,
                    s.className, s.shortName, isInitialized);
        ...
        if(className) {
            ...
            // it like all subscriptions of this class.

            // Send to all clients but the one with this socket.
            socket.Broadcast('advertise', s.id, s.name, s.className,
                    s.shortName, s.description, null/*javaScript URL*/);
        }
    });


    // Many network events have this same form for the webSocket on()
    // handlers.  They all just call the subscription object method with
    // the arguments that come from the arguments that are send in the
    // webSocket.  We set them all up here in this array.forEach() loop.
    //
    // Put another way: Network-ify these subscription methods.
    ['destroy', 'subscribe', 'unsubscribe', 'makeOwner'].
        forEach(function(eventName) {
            on(eventName, function(id) {
                if(subscription[id] === undefined)
                    // This is an old subscription that will never exist
                    // again.  The request is too late.  No big deal.
                    return;
                var args = [].slice.call(arguments);
                args.shift(); // remove the first arg which is id
                // Call the subscription object method with the
                // name of 'eventName'.
                (subscription[id][eventName])(...args);
            });
    });
